...
def inscription(request):
    ...
    if request.POST:
        if (request.POST['typeProfile'] == 'etudiant') and etudiant.is_valid() and compteEtudiant.is_valid():
            user = User.objects.create_user(username=request.POST['etud-pseudo'], email=request.POST['etud-email'],
                        password=request.POST['etud-password'], is_staff=False, is_active=False)
            etudiant.instance.compte = user
            etudiant.save()
        elif (request.POST['typeProfile'] == 'postulant') and postulant.is_valid() and formation.is_valid() and comptePostulant.is_valid() :
            user = User.objects.create_user(username=request.POST['post-pseudo'], email=request.POST['post-email'],
                        password=request.POST['post-password'], is_staff=False, is_active=False)
...
def ajax_email(request):
    ...
    """
    html = ''
    if (request.GET) and ('valeur' in request.GET):
        champ = forms.EmailField()
        try:
            champ.clean(request.GET['valeur'])
  ...
  <path d="M1.41 0l-1.41 1.41.72.72 1.78 1.81-1.78 1.78-.72.69 1.41 1.44.72-.72 1.81-1.81 1.78 1.81.69.72 1.44-1.44-.72-.69-1.81-1.78 1.81-1.81.72-.72-1.44-1.41-.69.72-1.78 1.78-1.81-1.78-.72-.72z" />
            ...
            try:
                User.objects.get(email = request.GET['valeur'])
  ...
  <path d="M3.09 0c-.06 0-.1.04-.13.09l-2.94 6.81c-.02.05-.03.13-.03.19v.81c0 .05.04.09.09.09h6.81c.05 0 .09-.04.09-.09v-.81c0-.05-.01-.14-.03-.19l-2.94-6.81c-.02-.05-.07-.09-.13-.09h-.81zm-.09 3h1v2h-1v-2zm0 3h1v1h-1v-1z" />
    ...
    return HttpResponse("""<p style='color: red;' class='message'> Problème de requête, veillez réessayer ultérieurement </p>""")
...
def ajax_pseudo(request):
  ...
  <path d="M1.41 0l-1.41 1.41.72.72 1.78 1.81-1.78 1.78-.72.69 1.41 1.44.72-.72 1.81-1.81 1.78 1.81.69.72 1.44-1.44-.72-.69-1.81-1.78 1.81-1.81.72-.72-1.44-1.41-.69.72-1.78 1.78-1.81-1.78-.72-.72z" />
            ...
            try:
                User.objects.get(username = request.GET['valeur'])
                html = """<p style='color: blue;' class="message"> <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 8 8">
  <path d="M3.09 0c-.06 0-.1.04-.13.09l-2.94 6.81c-.02.05-.03.13-.03.19v.81c0 .05.04.09.09.09h6.81c.05 0 .09-.04.09-.09v-.81c0-.05-.01-.14-.03-.19l-2.94-6.81c-.02-.05-.07-.09-.13-.09h-.81zm-.09 3h1v2h-1v-2zm0 3h1v1h-1v-1z" />
    ...
    if request.GET and ('dpt' in request.GET) and ('niveau' in request.GET):
        ...
        class ChampsFormation(forms.Form):
            if request.GET['niveau'] == 'master':
                formation = forms.ModelChoiceField(queryset=Formation.objects.filter(dpt = request.GET['dpt'] or None, niveau = request.GET['niveau'] or None), empty_label = '--------')
            elif request.GET['niveau'] == 'doctorat':
                theme_doc = forms.CharField(label='Thème', max_length=1000)
